...
### 3.6 睡眠 = 单信号 `energy` + 昼夜衰减
- [x] **不拆 `alertness`**（拆的理由随 `REFLEX_SIGNALS` 一起消失）
- [x] `energy` 衰减 **× 昼夜系数**（夜里掉得快）+ `activity_mul`（干活更快）
- [x] 归零 → `hp` 掉（现有行为保留）；睡觉恢复（床数据**不用改**）
- [x] **不新增信号**；`SIGNALS` 保持 `energy / hunger / bladder / hp`

### 3.7 生命（`hp`）三态
- [x] `hp ↓`：`hunger == 0` 或 `energy == 0`
- [x] `hp ↑`：`hunger ≥ 0.5` 且 `energy ≥ 0.5`
- [x] **其他 → hp 不动**（中间带，新增 `hp_regen_floor = 0.5`）
- [x] `bladder` **不参与** `hp`
- [x] `hp < 0.5` → 计划失去拉力 → **先满足自身需求**（`hp_override = 0.5`）
