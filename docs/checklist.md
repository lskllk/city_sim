...
> 本文件 = **现状**：定过什么 / 机制是什么 / 到哪了 / 下一步做什么。
> **玩法方向的设计真源已迁至 [设计决定](decisions.md)**（2026-09-18 架构访谈 30 问），冲突时以它为准。
...
| 10 | 情绪 / 关系 / 信任演化 | **不做** |
| 11 | 语义层上层 | **只做语域（persona × mood），不接 LLM**（2026-09-18 确认保留；LLM 只用于对手老板的经营决策）|
| 12 | Episode(L1 情景记忆) | **不做**（L0 物化记忆够用） |
| 13 | 生老病死 | **只剩饿死**（引擎本来就只支持饿死；~~累死~~ 已删，不做衰老/生育）|
| 14 | 推送性能优化（静态段/视口） | **先不做** |
| 15 | 编辑器编 `plans` | **不做** —— **游戏内部写死规则** |
| 16 | 归档文档 | **全删**（见 §4） |
...
**明确不做**：verify 校验 / 情绪 / 关系网 / trust 演化 / Episode / 衰老生育 / **LLM 润色（台词）** /
编辑器编 plans / 视口过滤 / `nn/` 学习层。

> **长线新增系统**（见 [设计决定 §四](decisions.md)）：账本 · 精度衰减器 · 经营决策接口 · LLM 驱动 ·
> 造人器 · 开局初始化 · 减员可感知 · 模板池表达维度表。
> **LLM 的唯一岗位**：对手老板的经营决策（藏在接口后面，可降级/可回放）；**台词不接 LLM**。
