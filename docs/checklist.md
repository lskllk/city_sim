...
| **传播/招牌** | 一对一搭桥（说×听×同屋/路人权重）；招牌 = 虚拟说者，门口半径、一条消息 1 tick |
...
| **招牌** | 建筑字段(公司/半径/相信度/最多 3 条)；**门口**半径内**按 told 那套搭桥**（广播频率 × 听概率）→ 停下、一条消息 1 tick 听完、气泡/事件都是 told；编辑器可挂、观察器可看 |
...
```
① ✅ 招牌 —— 数据 + 后端(路过即被 told) + 前端可视化 + 编辑器(已做)
