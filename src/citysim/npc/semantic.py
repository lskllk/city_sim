"""semantic —— NPC 说什么(措辞层)。

一句话: **先把真值攒成结构化事件, 再把它译成人话**。

因此两条硬规矩:
  · 随机只允许发生在【译成人话】这一步 —— 谁 / 知道什么 / 什么行为 /
    什么强度, 全是真值, 一个都不许编;
  · 只能说它【知道】的事(知识边界优先于表达力) —— 说出的话本身就
    是一条带来源的 Fact, 所以它同时是知识系统的可视化与传播载体。

做法是最小集: 手写组合式模板池(句子 = opener + core + tail), 选哪一段
由 speaker/act/topic/天 确定性地派生 —— 不掷骰子也不重复。不接 LLM。
