...
def main(n_npc: int = 3, seed: int = 7, days: int = 7) -> None:
    ticks = days * 1440
    ...
    n = n_npc
    print(f"\n总数: 睡眠段 {total} (人均{total / n:.1f}/{days}天 = "
          f"{total / n / days:.2f}/天)")
    ...
    print(f"实体数: {init_entities} -> {len(world.entities)} "
          f"(增长 {len(world.entities) - init_entities})")
